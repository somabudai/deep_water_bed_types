...
#' Log generator parameter hints
#'A function that gives information on sand-gravel fraction, sand thickness and number of beds to help setting parameters for log generation.
...
#'
#' @examples
#' log_param_hints(input_bedding_data = bed_data_dummy, 
...
log_param_hints <- function(input_bedding_data, input_ng_data, selected_sys_type = NA, selected_element, selected_climate = NA, selected_period = NA) {
  ...
  NG_value <- input_ng_data %>% 
    ...
    summarise(mean_ng = mean(NG), min_ng = min(NG), max_ng = max(NG))
  
  mean_ng <- NG_value %>% pull(mean_ng) %>% as.numeric()
  min_ng <- NG_value %>% pull(min_ng) %>% as.numeric()
  max_ng <- NG_value %>% pull(max_ng) %>% as.numeric()
  
  sand_thickness_bed_nr <- input_bedding_data %>% 
  ...
  mean_sand_thck <- mean(sand_thickness_bed_nr$sand_thck)
  min_sand_thck <- min(sand_thickness_bed_nr$sand_thck)
  max_sand_thck <- max(sand_thickness_bed_nr$sand_thck)
  
  mean_bed_n <- mean(sand_thickness_bed_nr$bed_nr)
  min_bed_n  <- min(sand_thickness_bed_nr$bed_nr)
  max_bed_n  <- max(sand_thickness_bed_nr$bed_nr)
  ...
  cat("Based on the input value the filtered dataset is characterised by the following values:\n",
               'The sand-gravel fraction falls between ', round(min_ng,2), ' and ', round(max_ng,2), ' with a mean of ' , round(mean_ng,2), '\n',
               'The sand thickness in specific logs falls between ', round(min_sand_thck,2), ' and ', round(max_sand_thck,2), ' with a mean of ' , round(mean_sand_thck,2), '\n',
               'The number of beds in specific logs falls between ', min_bed_n, ' and ', max_bed_n, ' with a mean of ' , round(mean_bed_n,2), '\n',
               'The mean bed thickness is ', round(average_bed_thickness,2), sep = '')
